...
% % Autocorrélation à 2Tau
% autoCorr2Tau = xcorr(sfFiltered(2:2:end), sfFiltered(1:2:end), 'unbiased');
% autoCorr2Tau = autoCorr2Tau(floor(length(autoCorr2Tau)/2)+1:end);
% tAutoCorr2Tau = (1/FsSF:1/FsSF:length(autoCorr2Tau)/FsSF)';
% 
% % Autocorrélation à 2Tau-1
% autoCorr2TauMoins = xcorr(sfFiltered(1:2:end), sfFiltered(1:2:end), 'unbiased');
% autoCorr2TauMoins = autoCorr2TauMoins(floor(length(autoCorr2TauMoins)/2)+1:end);
% tAutoCorr2TauMoins = (1/FsSF:1/FsSF:length(autoCorr2TauMoins)/FsSF)';
% 
% % Autocorrélation à 2Tau+1
% autoCorr2TauPlus = xcorr(sfFiltered(3:2:end), sfFiltered(3:2:end), 'unbiased');
% autoCorr2TauPlus = autoCorr2TauPlus(floor(length(autoCorr2TauPlus)/2)+1:end);
% tAutoCorr2TauPlus = (1/FsSF:1/FsSF:length(autoCorr2TauPlus)/FsSF)';
